...
automatically. These RNA panels are written to `Src/PaperFigures/figures/`; the ATAC
supplement (S5A-H, last section) is written to `Src/PaperFigures/panels/` by a separate
builder and is not governed by `PAPER_PANELS`. Shared methods (cell counts, normalisation, state definitions, pseudotime
...
`FigS15_Comp_perturbation_gene_heatmap_day04.pdf` · Fig4L_Fig5FG_S15__Perturbation_Gene_Effects.ipynb
**[day04 of 4L]**
...
6. **Run order.** The Pseudotime notebook
   ...
   `Fig4L_Fig5FG_S15__Perturbation_Gene_Effects.ipynb`, which read
...
and written to `Src/PaperFigures/panels/`, not `figures/`.
