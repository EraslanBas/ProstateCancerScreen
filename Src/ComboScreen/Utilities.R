...
extract_parts <- function(file_path) {
  fname <- basename(file_path)
  fname <- sub("_de_results\\.csv$", "", fname)
  parts <- strsplit(fname, "_")[[1]]
  perturbation <- parts[1]
  cell_count <- parts[2]
  umi_count <- parts[3]
  condition_label <- paste0(cell_count, "_", umi_count)
  list(perturbation = perturbation, condition_label = condition_label)
}
